...
for (const age of master.ages) {
  const competencies = semanticCatalog.competencies.map((competency) => ({
    id: competency.id, official_name: competency.official_name,
    official_review_status: competency.official_review_status ?? "pending", semantic_review_status: competency.semantic_review_status ?? "pending",
    meaning: competency.ai_meaning, pedagogical_intent: competency.pedagogical_intent ?? competency.ai_meaning,
    ...
    when_to_use: competency.when_to_use ?? competency.ai_selection_hints ?? [], do_not_use_when: competency.do_not_use_when ?? competency.avoid_when ?? [],
    typical_contexts: competency.ai_selection_hints ?? [], observable_actions: competency.ages[String(age)]?.observable_patterns ?? [],
    possible_evidence: competency.possible_evidence ?? [], examples: competency.examples ?? [], not_examples: competency.not_examples ?? [], key_signals: competency.ai_selection_hints ?? [], common_confusions: competency.common_confusions ?? [],
