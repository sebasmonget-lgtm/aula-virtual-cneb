...
test("aplica la política inicial de routing por workflow", () => {
  const cases = [
    ["activity", "standard_generation", "openai", "gpt-5.6-terra"],
    ["project", "deep_generation", "openai", "gpt-5.6-sol"],
    ["annual_plan", "deep_generation", "openai", "gpt-5.6-sol"],
    ["material_generation", "light_generation", "openai", "gpt-5.6-luna"],
  ];
  for (const [workflow, tier, provider, model] of cases) {
    assert.deepEqual(resolveAIExecutionPlan({ workflow }), {
      execution: "generation", tier, provider, model, capability: null,
...
test("mantiene evidence_capture y today_mode en código", () => {
  ...
  assert.deepEqual([explanation.execution, explanation.tier, explanation.model], ["generation", "light_generation", "gpt-5.6-luna"]);
});
...
test("usa TypeSafe solo para tareas de decisión estructurada", () => {
  ...
  assert.deepEqual(plan, {
    execution: "decision", tier: "decision", provider: "typesafe", model: null, capability: "decision",
