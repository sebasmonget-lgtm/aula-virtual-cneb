...
test("R-Y: metadata queda server-side, PUT conserva identidad y active se lee sin mutación", async () => {
  const server = await readFile(new URL("../../scripts/local-db-server.mjs", import.meta.url), "utf8");
  ...
  assert.match(server, /update activities set occurs_on=.*updated_at=now\(\) where id=\$6/);
