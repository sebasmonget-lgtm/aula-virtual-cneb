...
export function FamilyReportGenerator({ students }: { students: LocalStudent[] }) {
  ...
  useEffect(() => {
    ...
    })().catch((error: Error) => { if (live) { setMessage(error.message); setMessageTone("error"); setLoadingOptions(false); } }).finally(() => { if (live && periodStart && periodEnd) setLoadingOptions(false); });
    return () => { live = false; };
  ...
  return <section className="ayni-workflow space-y-5">
    ...
    {studentId && periodStart && periodEnd && !loadingOptions && <>
      ...
      {proposal && <div className="ayni-panel space-y-4 p-4 sm:p-6"><WorkflowFeedback tone={readOnly ? "success" : "info"}>{readOnly ? "Informe confirmado · solo lectura" : stored ? "Borrador del informe · revisa antes de confirmar" : "Propuesta de informe. Revísala antes de guardar."}</WorkflowFeedback>
        ...
        {readOnly ? <Button variant="outline" onClick={() => { setStored(null); setProposal(null); setGenerationId(null); }}>Preparar nueva versión</Button> : <div className="flex flex-wrap gap-2"><AsyncButton busy={operation === "save"} busyLabel="Guardando..." disabled={busy || !selectionMatches || !generationId && !stored} onClick={() => void save()}>{stored ? "Guardar cambios" : "Guardar borrador"}</AsyncButton><Button variant="outline" disabled={busy} onClick={() => { setProposal(stored?.details ?? null); setSelectedIds(stored?.selected_competency_ids ?? []); setGenerationId(null); }}>Descartar cambios</Button>{stored && <AsyncButton busy={operation === "confirm"} busyLabel="Confirmando..." disabled={busy || Boolean(generationId) || !selectionMatches} onClick={() => void confirm()}>Confirmar informe</AsyncButton>}</div>}
      </div>}
