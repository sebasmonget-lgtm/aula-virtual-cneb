...
export function AssessmentGenerator({ students, initialStudentId = "", initialCompetencyId = "", onNext, onPlan }: { students: LocalStudent[]; initialStudentId?: string; initialCompetencyId?: string; onNext?: (studentId: string, competencyId: string) => void; onPlan?: () => void }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setMessage("");
    }).catch((error: Error) => { if (live) { setContextError(true); setStored(null); setProposal(null); setTimeline([]); setLatest(null); setMessage(error.message); setMessageTone("error"); } }).finally(() => { if (live) setLoadingContext(false); });
    return () => { live = false; };
  }, [studentId, competencyId, start, end, contextReload]);
  ...
  return <section className="ayni-workflow space-y-5">
    ...
    <div className="ayni-panel space-y-4 p-4 sm:p-5">
      <label className="block">Niño<select value={studentId} onChange={(event) => { setStudentId(event.target.value); setLoadingOptions(Boolean(event.target.value)); setOptionsError(false); setContextError(false); setCompetencyId(""); setOptions([]); setStored(null); setProposal(null); setTimeline([]); setLatest(null); setMessage(""); }}><option value="">Selecciona un niño</option>{students.map((student) => <option key={student.id} value={student.id}>{student.name}</option>)}</select></label>
      {loadingOptions && <LoadingState label="Cargando competencias y evidencias..." />}
      {studentId && !loadingOptions && !optionsError && <label className="block">Competencia<select value={competencyId} onChange={(event) => { setCompetencyId(event.target.value); setLoadingContext(Boolean(event.target.value)); setContextError(false); setStored(null); setProposal(null); setTimeline([]); setLatest(null); setMessage(""); }}><option value="">Selecciona una competencia</option>{options.map((option) => <option key={option.competency_v4_id} value={option.competency_v4_id}>{option.name} · {option.evidence_count} evidencias</option>)}</select></label>}
      {optionsError && <Button variant="outline" onClick={() => { setLoadingOptions(true); setOptionsReload((value) => value + 1); }}>Reintentar carga de competencias</Button>}
    </div>
    ...
    {studentId && !loadingOptions && !optionsError && options.length === 0 && <EmptyState title="Sin competencias con evidencias" description="Registra una observación desde una actividad con criterio confirmado para iniciar el análisis." action={onPlan && <Button variant="outline" onClick={onPlan}>Ver actividades</Button>} />}
    {message && <WorkflowFeedback tone={messageTone}>{message}</WorkflowFeedback>}
    {competencyId && <div className="space-y-5">
      <div className="ayni-panel space-y-4 p-4 sm:p-5"><h3 className="font-bold">{selected?.name}</h3><div className="grid gap-3 sm:grid-cols-2"><label>Desde<input type="date" value={start} onChange={(event) => { setStart(event.target.value); setLoadingContext(Boolean(event.target.value && end)); }} /></label><label>Hasta<input type="date" value={end} onChange={(event) => { setEnd(event.target.value); setLoadingContext(Boolean(start && event.target.value)); }} /></label></div>{!contextError && <p className="text-sm text-[#526b87]">{timeline.length} evidencias en este periodo.</p>}{latest && !contextError && <p className="text-sm text-[#526b87]">Último análisis confirmado: {latest.period_start} a {latest.period_end} · {latest.details.information_status === "sufficient" ? "Información suficiente" : "Información insuficiente"}</p>}</div>
      ...
      {!proposal ? <AsyncButton busy={operation === "generate"} busyLabel="Analizando las evidencias..." disabled={busy || loadingContext || contextError || timeline.length === 0} onClick={() => void generate()}>Preparar análisis</AsyncButton> : <div className="ayni-panel space-y-4 p-4 sm:p-6">
        ...
        {readOnly ? <Button variant="outline" onClick={() => { setStored(null); setProposal(null); setGenerationId(null); }}>Preparar nueva versión</Button> : <>{hasUnsavedChanges && <p className="text-sm text-[#526b87]">Guarda los cambios antes de confirmar.</p>}<div className="flex flex-wrap gap-2"><AsyncButton busy={operation === "save"} busyLabel="Guardando..." disabled={busy} onClick={() => void save()}>{stored ? "Guardar cambios" : "Guardar borrador"}</AsyncButton><AsyncButton variant="outline" busy={operation === "generate"} busyLabel="Regenerando..." disabled={busy} onClick={() => void generate()}>Regenerar</AsyncButton><Button variant="outline" disabled={busy} onClick={() => { setProposal(stored?.details ?? null); setGenerationId(null); }}>Descartar cambios</Button>{stored && <AsyncButton busy={operation === "confirm"} busyLabel="Confirmando..." disabled={busy || hasUnsavedChanges || contextError} onClick={() => void confirm()}>Confirmar análisis</AsyncButton>}</div></>}
      </div>}
      {!loadingContext && !contextError && (readOnly || latest) && onNext && <NextStepCard title="Análisis confirmado disponible" description={readOnly ? "Ya puedes preparar una conclusión a partir de este análisis." : "El borrador nuevo no impide continuar con el análisis confirmado anterior."} action="Preparar conclusión" onAction={() => onNext(studentId, competencyId)} />}
    </div>}
  </section>;
}
