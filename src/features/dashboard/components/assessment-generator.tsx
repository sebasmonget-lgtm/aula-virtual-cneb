...
export function AssessmentGenerator({ students }: { students: LocalStudent[] }) {
  ...
  useEffect(() => {
    ...
    }).then((data) => { if (live) { setOptions(data.competencies); setStart(data.calendar.starts_on.slice(0, 10)); setEnd(data.calendar.ends_on.slice(0, 10)); setLoadingContext(true); setOptionsError(false); setMessage(""); } }).catch((error: Error) => { if (live) { setOptionsError(true); setMessage(error.message); setMessageTone("error"); } }).finally(() => { if (live) setLoadingOptions(false); });
    return () => { live = false; };
  ...
  return <section className="ayni-workflow space-y-5">
    ...
    {studentId && !loadingOptions && !optionsError && options.length === 0 && <EmptyState title="Sin competencias con evidencias" description="Registra una observación desde una actividad con criterio confirmado para iniciar el análisis." />}
    ...
    {competencyId && <div className="space-y-5">
      ...
      {!proposal ? <AsyncButton busy={operation === "generate"} busyLabel="Analizando las evidencias..." disabled={busy || loadingContext || contextError || timeline.length === 0} onClick={() => void generate()}>Analizar evidencias con IA</AsyncButton> : <div className="ayni-panel space-y-4 p-4 sm:p-6">
