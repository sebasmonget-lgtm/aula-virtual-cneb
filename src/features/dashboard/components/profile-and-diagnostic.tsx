  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
      <section className="diagnostic-panel p-5 md:p-7">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3"><Button onClick={save} disabled={working}><Save /> {working ? "Guardando..." : "Guardar perfil"}</Button>{message && <span role="status" className="text-sm text-[#1f625c]">{message}</span>}</div>
      </section>
...
export function GuidedDiagnostic({ dashboard }: { dashboard: LocalDashboard }) {
  ...
  if (!data) return <p className="text-muted-foreground">Cargando diagnóstico...</p>;
  ...
  return <div className="diagnostic-shell space-y-6">
    ...
    {step === 2 && selectedGuide && <section className="diagnostic-panel p-5 md:p-7">
      ...
      <div className="mt-6 space-y-3"><h3 className="font-bold">Qué observar</h3>{references.map((reference) => <div key={reference.id} className={`rounded-xl border p-4 ${referenceId === reference.id ? "border-[#6eb9ca] bg-[#f0f9fc]" : ""}`}><label className="flex cursor-pointer gap-3"><input type="radio" name="reference" checked={referenceId === reference.id} onChange={() => setReferenceId(reference.id)} /><span className="font-medium">{reference.short_observable_text}</span></label>{referenceId === reference.id && <div className="mt-4 grid gap-2 sm:grid-cols-2">{states.map(([value, label]) => <button key={value} type="button" onClick={() => setReferenceStatus(value)} className={`rounded-xl border px-3 py-3 text-left text-sm ${referenceStatus === value ? "border-[#087d96] bg-[#daf3f7] font-semibold" : "bg-white"}`}>{referenceStatus === value && <Check className="mr-1 inline size-4" />}{label}</button>)}</div>}</div>)}</div>
      ...
      <div className="sticky bottom-16 mt-5 flex flex-wrap gap-3 border-t bg-white py-4 md:bottom-0"><Button onClick={() => save(false)} disabled={!referenceId || !referenceStatus || working}><Save /> Guardar</Button><Button variant="outline" onClick={() => save(true)} disabled={!referenceId || !referenceStatus || working}>Guardar y siguiente <ArrowRight /></Button></div>
    </section>}
