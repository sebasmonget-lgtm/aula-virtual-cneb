...
export function StudentsScreen({ students, onImported, onEvaluate, onPlan, onDiagnostic }: { students: LocalStudent[]; onImported: (dashboard: LocalDashboard) => void; onEvaluate?: (studentId: string, stage: ReviewStage, competencyId: string) => void; onPlan?: () => void; onDiagnostic?: () => void }) {
...
function DiagnosticHistory({ observations, diagnosis, confirmed }: {
  ...
  if (!observations.length && !diagnosis.length && !confirmed.length) return <Empty message="No hay registros de diagnóstico para este niño." />;
  return <div className="space-y-2">
    {confirmed.map((item) => <article key={item.id} className="rounded-2xl border border-[#b5dfc8] bg-[#f0faf4] p-4">
    ...
    {observations.map((item) => <article key={item.id} className="rounded-2xl border bg-white p-4">
      <p className="font-semibold">{item.competency_name}</p>
