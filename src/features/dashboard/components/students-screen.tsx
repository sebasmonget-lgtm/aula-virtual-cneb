...
export function StudentsScreen({ students, onImported }: { students: LocalStudent[]; onImported: (dashboard: LocalDashboard) => void }) {
  ...
  async function addStudents(useCsv: boolean) {
    ...
    try {
      ...
      setImportMessage("Niños añadidos al aula.");
    } catch (error) { setImportMessage(error instanceof Error ? error.message : "No se pudieron añadir los niños."); }
  ...
  return <section className="mx-auto max-w-4xl space-y-5"><div><p className="text-sm font-semibold text-[#087d96]">Aula activa</p><h1 className="mt-1 text-3xl font-extrabold">Niños</h1></div><div className="space-y-3 rounded-2xl border bg-white p-5"><h2 className="font-bold">Añadir niños</h2><div className="grid gap-2 sm:grid-cols-3"><input className="rounded border p-2" aria-label="Nombre" placeholder="Nombre" value={firstName} onChange={(event) => setFirstName(event.target.value)} /><input className="rounded border p-2" aria-label="Apellido" placeholder="Apellido" value={lastName} onChange={(event) => setLastName(event.target.value)} /><input className="rounded border p-2" aria-label="Nombre preferido" placeholder="Nombre preferido (opcional)" value={preferredName} onChange={(event) => setPreferredName(event.target.value)} /></div><Button disabled={importBusy || !firstName.trim() || !lastName.trim()} onClick={() => void addStudents(false)}>Añadir niño</Button><details><summary className="cursor-pointer font-semibold">Importar varios desde CSV</summary><p className="mt-2 text-sm text-muted-foreground">Encabezado: first_name,last_name,preferred_name. Una fila por niño; máximo 40 por importación.</p><Textarea className="mt-2" value={csv} onChange={(event) => setCsv(event.target.value)} placeholder={'first_name,last_name,preferred_name\nMaría,López,María'} /><Button className="mt-2" variant="outline" disabled={importBusy || !csv.trim()} onClick={() => void addStudents(true)}>Importar CSV</Button></details>{importMessage && <p role="status" className="text-sm">{importMessage}</p>}</div><label className="flex h-12 items-center gap-2 rounded-2xl border bg-white px-4 text-[#60718a]"><Search className="size-5" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar niño" className="min-w-0 flex-1 bg-transparent outline-none" /></label><div className="space-y-2">{visibleStudents.length ? visibleStudents.map((student) => <button key={student.id} type="button" onClick={() => { setProfile(null); setLoading(true); setSelectedId(student.id); }} className="flex w-full items-center justify-between rounded-2xl border bg-white px-5 py-4 text-left transition hover:border-[#8fd4dc] hover:bg-[#f3fcfc]"><span className="font-bold">{student.name}</span><span className="text-sm font-semibold text-[#087d96]">Ver perfil →</span></button>) : <p className="rounded-xl border border-dashed p-4 text-sm">Aún no hay niños en el aula. Añade uno para comenzar.</p>}</div></section>;
}
...
function StudentProfile({ profile, loading, error, onBack }: { profile: StudentPedagogicalProfile | null; loading: boolean; error: string; onBack: () => void }) {
  ...
  if (loading || !profile) return <section className="mx-auto max-w-4xl space-y-3"><Button variant="ghost" onClick={onBack}><ArrowLeft /> Volver a Niños</Button><p role="status" className="text-sm text-muted-foreground">{error || "Cargando perfil pedagógico…"}</p></section>;
