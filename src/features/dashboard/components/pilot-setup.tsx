...
export function PilotSetup({ onReady }: { onReady: (dashboard: LocalDashboard) => void }) {
  ...
  return <main className="mx-auto max-w-2xl space-y-5 p-5 md:p-9"><header><p className="font-semibold text-[#087d96]">Primeros pasos</p><h1 className="text-3xl font-bold">Configura tu aula</h1><p className="mt-2 text-sm text-muted-foreground">Después podrás añadir los niños desde la sección Niños y comenzar a planificar.</p></header>
    ...
    {error && <p role="alert" className="text-sm text-destructive">{error}</p>}<Button disabled={busy} onClick={() => void save()}>{busy ? "Guardando…" : "Crear aula"}</Button></main>;
}
