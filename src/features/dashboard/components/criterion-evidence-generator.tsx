...
export function CriterionEvidenceGenerator({ activityId, activityTitle, competencyName }: { activityId: string; activityTitle: string; competencyName: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(`${localDatabaseApiUrl}/api/activity-criteria?activityId=${activityId}`)
      ...
      .catch(() => { if (!cancelled) { setStored(null); setProposal(null); setLoaded(true); setMessage("No se pudo cargar el criterio de esta actividad."); setMessageTone("error"); } });
  ...
  if (!loaded) return <LoadingState label="Cargando criterio y evidencia..." />;
