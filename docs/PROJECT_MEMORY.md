...
- La jerarquía vigente de producción es `annual_plan` → `learning_experience` (`project` | `unit`; `workshop` futuro) → `activity` → `criterion_and_evidence` → `evidence_capture` → `assessment` → `descriptive_conclusion` → `family_report` futuro. El plan anual propone; la docente confirma. Project y Unit se materializan como `learning_experiences`; sus borradores son reabribles y se revalidan antes de guardar o confirmar, mientras las experiencias activas son de solo lectura.
- Activity v4 nace desde un Project o Unit activo elegido por la docente en Planificar → Actividades. El servidor carga el contexto pedagógico del parent, edad, aula, diagnóstico disponible, calendario y aplicabilidad lingüística; la actividad solo pide fecha, propósito puntual, competencia cuando corresponda, materiales adicionales y contexto del día. El draft es reabrible y la confirmación lo vuelve de solo lectura. La competencia CNEB v4 se conserva en `activities.details`; `activity_criteria` legacy no se usa ni recibe mapeos en esta fase. `criterion_and_evidence` será el siguiente workflow.
...
- Siete workflows generan mediante modelo: `annual_plan`, `project`, `unit`, `activity`, `criterion_and_evidence`, `assessment` y `descriptive_conclusion`. `evidence_capture` sigue siendo determinista en código. Los otros workflows aún no generan. Supabase, despliegue, PDFs, embeddings, vector DB y Jev legacy siguen fuera de esta capa.
...
1. Diseñar `family_report` a partir de conclusiones descriptivas confirmadas mediante `generateAIWorkflowV4()` y conservar la confirmación docente de toda propuesta.
