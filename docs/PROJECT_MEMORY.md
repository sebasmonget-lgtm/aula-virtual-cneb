...
- Existe una capa de generación v4 para `activity`, `annual_plan`, `project` y `unit`: `generateAIWorkflowV4()` entrega solo una copia inmutable del `AIContextBundle` y valida una salida estructurada antes de devolverla. `OpenAIProvider` usa la Responses API con Structured Outputs strict cuando recibe `OPENAI_API_KEY`; no hay clave en el repositorio, no hay llamadas durante pruebas y no se habilitaron otros proveedores externos. `createAIProviderForPlan()` centraliza su construcción y `npm run smoke:openai-activity` deja preparada una prueba real ficticia que no se ejecuta en CI ni durante desarrollo normal.
