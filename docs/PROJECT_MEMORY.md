...
- La jerarquía vigente de producción es `annual_plan` → `learning_experience` → `activity` → evidencia/evaluación. El plan anual se guarda como borrador y exige confirmación docente; sus experiencias quedan como propuestas hasta implementar sus workflows.`r`n- La pantalla Planificar integra `activity` mediante el backend local: precarga aula, edad y materiales, recibe solo una propuesta validada editable y no muestra metadata técnica. La propuesta no se guarda automáticamente; falta una operación local de creación de actividades aprobada antes de persistirla.
